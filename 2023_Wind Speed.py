...
def plot_wind_rain_scatter_2023(file_path):
    """
    This function creates a scatter plot to show the relationship between 
    average wind speed and total rainfall for each day of 2023, 
    ...
    plt.title('Wind Speed vs. Rain (mm) (Daily Summary 4:00 PM - 6:00 PM) in 2023', fontsize=16)
    plt.xlabel('Average Wind Speed (km/h)', fontsize=12)
    plt.ylabel('Total Rainfall (mm)', fontsize=12)
    plt.grid(True, linest'--', alpha=0.6)
    plt.tight_layout()
    plt.show()
...
plot_wind_rain_scatter_2023(file_name)