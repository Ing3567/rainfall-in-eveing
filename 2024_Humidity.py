...
def plot_humidity_rain_scatter_2024(file_path):
    """
    This function creates a scatter plot to show the relationship between 
    average relative humidity and total rainfall for each day of 2024, 
    ...
    daily_summary = df_filtered_time.groupby(df_filtered_time['time'].dt.date).agg({
        'rain (mm)': 'sum',
        'relative_humidity_2m (%)': 'mean' # <<< เปลี่ยนเป็นคอลัมน์ความชื้น
    }).reset_index()
    ...
    sns.regplot(
        data=daily_summary, 
        x='relative_humidity_2m (%)',  # <<< แกน X คือ ความชื้น
    ...
    plt.title('Relative Humidity vs. Rain (mm) (Daily Summary 4:00 PM - 6:00 PM) in 2024', fontsize=16)
...
plot_humidity_rain_scatter_2024(file_name)