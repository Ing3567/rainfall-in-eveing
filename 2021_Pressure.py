...
def plot_pressure_rain_scatter_2021(file_path):
    """
    This function creates a scatter plot to show the relationship between 
    average mean sea level pressure and total rainfall for each day of 2021, 
    ...
    # --- 2. Daily Data Aggregation (CHANGE: Use pressure_msl) ---
    ...
    daily_summary = df_filtered_time.groupby(df_filtered_time['time'].dt.date).agg({
        'rain (mm)': 'sum',
        'pressure_msl (hPa)': 'mean'  # <<< เปลี่ยนเป็นคอลัมน์ความกดอากาศ
    }).reset_index()
    ...
    sns.regplot(
        data=daily_summary, 
        x='pressure_msl (hPa)',   # <<< แกน X คือ ความกดอากาศ
        y='rain (mm)',                  
    ...
    # Update plot title, xlabel
    plt.title('Pressure (MSL) vs. Rain (mm) (Daily Summary 4:00 PM - 6:00 PM) in 2021', fontsize=16)
...
plot_pressure_rain_scatter_2021(file_name)